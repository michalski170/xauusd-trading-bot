...
async def run_bot():
    ...
    print('Polaczono. Startuje bota...')

    while True:
        try:
            candles = await connection.get_candle_history(SYMBOL, TIMEFRAME, 500)
            ...
            if not positions:
                ...
                pos = positions[0]
                if pos['type'] == 'POSITION_TYPE_BUY' and is_short_signal(heikin_ashi):
                    print('Zamykam LONG')
                    await connection.close_position(pos['id'])
                elif pos['type'] == 'POSITION_TYPE_SELL' and is_long_signal(heikin_ashi):
                    print('Zamykam SHORT')
                    await connection.close_position(pos['id'])
