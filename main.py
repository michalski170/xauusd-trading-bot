...
async def run_bot():
    ...
    while account.connection_status != 'CONNECTED' or account.state != 'DEPLOYED':
    ...
    while True:
        try:
            candles = await connection.get_history({
                'symbol': SYMBOL,
                'period': TIMEFRAME,
                'startTime': (datetime.utcnow() - timedelta(minutes=500)).isoformat()
            })
